...
# How many epochs are allowed for each domain for the large-scale model training
MAXIMUM_USAGE = 15

# Minimum sampling rate to ensure statistical significance
MINIMUM = 2e-4

def get_token_distribution():
    # Wiki40B dataset language distribution for training
    ...
    # Validation languages: pl, uk, nl, pt, ca, tr, da, ro
    # All with equal weight (1.0)
    valid = {
        "valid_wiki40b_pl": 1.0,  # Polish
        "valid_wiki40b_uk": 1.0,  # Ukrainian
        "valid_wiki40b_nl": 1.0,  # Dutch
        "valid_wiki40b_pt": 1.0,  # Portuguese
        "valid_wiki40b_ca": 1.0,  # Catalan
        "valid_wiki40b_tr": 1.0,  # Turkish
        "valid_wiki40b_da": 1.0,  # Danish
        "valid_wiki40b_ro": 1.0,  # Romanian
    }
...
def generate_train_group(groups, weights, precision=5):
    ...
    precision (int, optional): Number of decimal places for rounding weights. Defaults to 5.
...
def generate_config_from_prior(output_paths, prior_config):
    ...
    prior_dist = np.array(prior_dist) / np.sum(prior_dist)
    ...
    for output_path, weights in zip(output_paths, train_weights):
        ...
        with open(output_path, "w", encoding="utf8") as f:
            ...
            content = ""
            content += "\n" + "model_name: wiki40b_multilingual"
            content += "\n" + "total_devices: 1"
            content += "\n" + "num_of_devices: 1"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_folder", type=str, default="config_1m_wiki40b")
    parser.add_argument("--num_configs", type=int, default=768)
