...
def load_dataset_samples(dataset_name, seed=42, num_samples = None):
    ...
    start_indices = np.zeros(total_examples, dtype=np.int64)
    current_idx = 0
    for i in range(total_examples):
        start_indices[i] = current_idx
        current_idx += int(train_lengths[i])  # Convert to int to avoid overflow

    # Debug: Print start indices information
    logger.debug(f"First 10 start indices: {start_indices[:10]}")
    logger.debug(f"Total tokens: {current_idx}")
    logger.debug(f"\n--- {dataset_name.upper()} Dataset Samples ---")
    
    for idx in tqdm(indices, desc=f"Loading {dataset_name} samples"):
        start_idx = start_indices[idx]
        length = int(train_lengths[idx])  
        end_idx = start_idx + length
        
        # Get tokens for this example
        sample_tokens = train_data[start_idx:end_idx]
       
        sample_text = decode_tokens_to_text(sample_tokens, tknzr)
        samples.append(sample_text)

    # check if sample seem ok
    for i, sample in enumerate(samples[:5], 1):
            logger.debug(f"\nSample {i} (length {len(sample)}):")
            logger.debug(sample)
            logger.debug("-" * 50)
    logger.debug(f"Total samples extracted: {len(samples)}")

    logger.setLevel(logging.INFO) # NOTE : Reset to default logging level
