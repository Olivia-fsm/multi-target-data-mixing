...
class DoGETrainer(Trainer):
    def __init__(self, *args, domain_args, 
        ...
        # self.sample_dw = self.domain_config.train_dw
        ...
        # if 'mix' in tgt_domains:
        #     self.eval_domain_list = self.domain_config.domain_list
        # else:
        #     self.eval_domain_list = tgt_domains
        self.sampling_weights = self.train_dw

        self.reweight_eps = self.args.reweight_eps
        self.doge = self.args.reweight_domains # bool
        self.doremi = self.args.doremi # bool
        ...
        if self.args.domain_update_per_iter is not None:
            ...
            # self.domain_update_counter = {i:0 for i in self.train_ids.tolist()}
    ...
    # def set_sample_dw(self):
    #     assert self.curriculum is not None, "No curriculum specificed!"
    #     assert self.state.global_step in self.curriculum.keys()
    #     new_sample_dw = torch.tensor(self.curriculum[self.state.global_step], dtype=torch.float)
    #     self.train_dataset._ex_iterable.probabilities_handle = new_sample_dw
    #     self.train_dataset._ex_iterable.probabilities = new_sample_dw
    #     # self.train_dataset = interleave_datasets(
    #     #                     self.train_dataset_ls,
    #     #                     probabilities=new_sample_dw,
    #     #                     probabilities_handle=new_sample_dw,
    #     #                     seed=self.args.seed)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, return_pertoken_losses=False):
        ...
        #     try:
        #         inputs[k] = v.to(model.module.device)
        #     except:
    ...
    def update_domain_weights_doremi(self, domain_losses_distributed, ref_domain_losses_distributed):
        ...
        # import pdb
        # pdb.set_trace()
        # print("domain loss: ", domain_losses_distributed)
        # print("ref loss: ", ref_domain_losses_distributed)
        # print("excess loss: ", excess_losses)
        excess_losses = torch.clip(excess_losses, min=0.0)
        # print("clip excess loss: ", excess_losses)
    ...
    def _inner_training_loop_curriculum(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            while self.state.global_step < steps_in_epoch:
                ...
                #     new_sample_dw = torch.tensor(self.curriculum[self.state.global_step], dtype=torch.float)
                #     epoch_iterator.dataset._ex_iterable.probabilities_handle = new_sample_dw
                #     epoch_iterator.dataset._ex_iterable.probabilities = new_sample_dw
                #     # self.train_dataset = interleave_datasets(
                #     #                     self.train_dataset_ls,
                #     #                     probabilities=new_sample_dw,
                #     #                     probabilities_handle=new_sample_dw,
...
# ## test ##
# data_config = DataTrainingArguments(dataset='redpajama-all',
#                                     max_train_samples=None,
#                                     max_eval_samples=None)
# train_ds, val_ds, domain_config, tokenizer = get_train_eval_datasets(data_config=data_config,
#                                                         verbose=True,
...
# training_args = FullTrainingArguments(output_dir="/mloraw1/sfan/doge/exp/test_hf",
#                                   do_train=True,
#                                   do_eval=True,
#                                   learning_rate=1e-4,
