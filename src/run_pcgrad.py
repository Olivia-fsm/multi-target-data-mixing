...
args_parser.add_argument('--config_json', default='../config/doge_82.json', type=str)
args_parser.add_argument('--wandb_proj', default='multi-target-multi-domain', type=str)
...
class GPUTimeCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step % args.logging_steps == 0:
            gpus = GPUtil.getGPUs()
            gpu_info = [{"id": gpu.id, "mem_used_percent": gpu.memoryUtil * 100} for gpu in gpus]
            elapsed = time.time() - self.start_time
            
            print(f"Step {state.global_step}: {elapsed/3600:.2f} hours, GPU: {gpu_info}")
            if args.local_rank == 0:
                wandb.log({
                    "elapsed_hours": elapsed / 3600,
                    **{f"gpu_{gpu['id']}_mem_percent": gpu["mem_used_percent"] for gpu in gpu_info}
                }, commit=False)
...
def main():
    ...
    if 'LOCAL_WORLD_SIZE' not in os.environ:
        os.environ['LOCAL_WORLD_SIZE'] = '1'  # Set to number of local GPUs
    ...
    trainer = MAPTrainerExtended(
        ...
        ref_model=ref_model,
        callbacks=[TqdmCallback(), GPUTimeCallback()],
    )
    ...
    print('PCGrad training completed! 🎯✨')
